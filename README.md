...
本项目最初设计支持线上部署（后端 Render + 前端 Vercel），但在实际部署过程中遇到以下问题：

1. **跨域配置冲突**：前端请求携带凭证时与后端 CORS 配置不一致，导致跨域预检失败
2. **接口路径不匹配**：前端登录/注册请求缺少 `/api` 前缀，与后端路由不匹配
3. **免费平台限制**：Render 免费实例存在资源限制和数据持久化问题（重启清空）
4. **环境变量配置复杂**：Vercel 环境变量与本地开发环境差异较大，调试困难

**当前状态**：项目已恢复为纯本地开发模式，CORS 配置为最简通用模式，可直接在本地运行。

如需尝试线上部署，请参考 `docs/部署指南.md`，但建议在本地开发完成后再进行部署。
